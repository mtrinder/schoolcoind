...
static const char *strMainNetDNSSeed[][3] = {
    {"Seed A", "a.seed.school-coin.org"},
    {"Seed B", "b.seed.school-coin.org"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Schoolcoin is probably already running."), addrBind.ToString().c_str());
        else
